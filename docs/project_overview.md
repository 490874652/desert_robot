...
创建 Isaac USD stage，生成更平滑、范围更大的程序化沙漠地形，加入地面参考线、障碍、车辆占位体、目标点、灯光和相机，并保存 USD。
...
在地表上方添加低矮参考线，帮助判断车辆是否贴地、路径是否沿正确方向运动。
...
对程序化高度图做轻量平滑，减少不必要的棱角，让沙地表面更连续。

`_sample_elevation(...)`

从 heightfield 中按世界坐标双线性采样高度，用于摆放地面参考线。
...
解析 USD 场景、`local_path.npz`、演示编号、小车速度、运行帧数和是否循环路径。

`main()`

启动 Isaac GUI，打开沙漠 USD 场景，选择 demo 路径，在场景中绘制路径点、连续路径带和车头方向箭头，并创建 `/World/LiveRover` 四轮底盘模型沿路径运动。默认到终点后停止；需要循环时加 `--loop`。`/World/LiveRover` 是整体移动的根节点，底盘、传感器立柱、轮子和轮毂都是根节点下的局部子部件，因此车轮不会和车架分离。四轮地形采样只用于估计整车姿态，根节点中心严格沿路径插值移动。
...
```bash
# 1 号：当前 A* 端点到端点路径
python isaac/live_rover_path_demo.py --demo 1

# 2 号：从低处到高处的爬坡路径
python isaac/live_rover_path_demo.py --demo 2
```
...
```text
Isaac GUI 中可以看到沙漠场景、地面参考线、四轮 rover、蓝色起点圆盘、蓝色连续路径带、黄色路径圆盘、红色终点旗帜和红色车头箭头。
...
根据 `--demo` 选择路径：`1` 读取当前 A* path，`2` 生成爬坡 path。

`_load_planned_path(...)`

读取 `local_path.npz`，把路径从局部 costmap 行列坐标转换成 Isaac 世界坐标。

`_make_uphill_path(...)`

从地形最低点到最高点生成一条爬坡演示路径。

`_load_terrain_sampler(...)`

读取 `/World/DesertTerrain` mesh 顶点，构建可双线性插值的地形高度采样器。

`_TerrainSampler.sample(...)`

根据世界坐标 `x, y` 从真实 USD 地形上采样高度；路径末端轻微越界时会夹到地形边界。

`_TerrainSampler.normal(...)`

根据地形高度差估算当前位置的表面法线。

`_create_live_rover(...)`

隐藏旧的方块占位车，创建 `/World/LiveRover`，包含底盘、传感器立柱、四个轮子和四个轮毂。

`_create_transform_op(...)`

为 rover 根节点创建单一 transform op。运动时直接写 4x4 矩阵，避免欧拉角旋转顺序导致车头方向和 path 切线不一致。

`_create_static_part_pose(...)`

为底盘、传感器、轮子、轮毂设置固定局部坐标。它们不再每帧独立更新世界坐标，而是跟随 `/World/LiveRover` 根节点整体运动。

`_wheel_local_positions(...)`

返回四个轮子在车体局部坐标系中的固定位置。

`_set_live_rover_pose(...)`

根据路径中心点和路径方向更新 rover 根节点：先计算四轮接触点，再估计整车姿态，最后只写入 `/World/LiveRover` 根节点的 4x4 矩阵。

`_wheel_contacts(...)`

根据四个轮子的局部坐标、车体中心和路径方向，估算四个轮子的世界接触点，并从地形采样每个轮子的接触高度。

`_normal_from_wheel_contacts(...)`

根据四轮接触点估计底盘支撑平面的法线。

`_vehicle_transform(...)`

显式构造 rover 根节点矩阵：前向轴对齐 path 切线，右向轴由前向轴和地面法线计算，上向轴对齐四轮接触平面的法线。

`_normalize_xy(...)`

把 path 方向向量归一化为水平 XY 方向。

`_draw_path_markers(...)`

在 Isaac 场景中绘制蓝色起点圆盘、蓝色连续路径带、黄色路径圆盘和红色终点旗帜。

`_draw_path_ribbon(...)`

把相邻 path 点连接成低矮蓝色路径带，路径带高度从真实地形采样并抬高，避免被沙地遮挡。

`_draw_disk(...)`

创建低矮圆盘标记，并设置位置、半径和颜色。

`_draw_flag(...)`

创建终点旗帜。

`_hide_legacy_goal_marker(...)`

隐藏旧的绿色球形目标点，避免和灌木或植物混淆。

`_set_active_camera()`

把 GUI 视口切换到 `/World/Camera`，方便打开后直接看到当前沙漠场景。

`_set_overhead_camera(...)`

把 `/World/Camera` 放到路径中心正上方，默认用完全俯瞰视角观察路径和车辆。
